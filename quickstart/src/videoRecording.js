  ...
  startRecording(nprId) {
    ...
    this.recorder.onstop = (evt) => {
      this.onRecordingEnded(
        new Blob(this.recordedChunks, { type: "video/webm" }, Date.now())
      );
      this.recorderChunks = null; // Remove reference
      if (process.env.REACT_APP_TIMED_VIDEO_DOWNLOAD === "true")
        clearTimeout(timedDownloader);
    };
